...
#define token @"fefaab1c7db08ed76310d619240dfdee"
...
NSString *searchRequestQuery(SearchRequest request) {
    ...
    if (request.departDate && request.returnDate) {
        ...
        result = [NSString stringWithFormat:@"%@&depart_date=%@&return_date=%@", result, [dateFormater stringFromDate:request.departDate], [dateFormater stringFromDate:request.returnDate]];
    }
    return result;
}

-(void)ticketsWithRequest:(SearchRequest)request
           withCompletion:(void (^)(NSArray * _Nonnull))completion
{
    NSString *urlString = [NSString stringWithFormat:@"%@?%@&token=%@", API_URL_CHEAP, searchRequestQuery(request), token];
